...
static struct timer_list timer_relay;

static unsigned int gpioLED = 12;       ///< hard coding the LED gpio for this example to P9_23 (GPIO49)
...
static int __init ebbgpio_init(void){
   ...
   //    return -ENODEV;
   // }
   // timer_setup(&timer_relay, timer_callback, 0);
   // mod_timer(&timer_relay, jiffies + msecs_to_jiffies(INTERVAL_TIME));
   ...
   //                      "tm-gpio-handler",     // Used in /proc/interrupts to identify the owner
   //                      NULL);
   //                   // The *dev_id for shared interrupt lines, NULL is okay
   // printk(KERN_INFO "GPIO_TEST: DEBUG, line = %i \n", 63);
   // if(result < 0){
   //    free_irq(irqNumber, NULL);
   //    gpio_unexport(gpioButton);
   //    gpio_free(gpioButton);
   //    printk(KERN_INFO "INVALID IRQ NUMBER %i \n", result);
   //    return -ENODEV;
   // }
